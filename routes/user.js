    ...
    try {
        ...
        if (resident) {
            ...
            if (resident.profile_pic && fs.existsSync(`./public/uploads/${resident.profile_pic}`)) {
                linkSfs.unync(`./public/uploads/${resident.profile_pic}`);
            }
...
router.get('/:userId/resident', authenticateToken, async (req, res) => {
    try {
        const userId = req.params.userId;

        // Find the resident associated with the user_id
        const resident = await Resident.findOne({
            where: { user_id: userId }
        });

        if (resident) {
            res.status(200).json({ name: resident.name });
        } else {
